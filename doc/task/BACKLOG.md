...
**Commit `f9d0ed5`.** Bỏ `alarmManager.set()` (inexact), dùng `setExactAndAllowWhileIdle()` khi có quyền `SCHEDULE_EXACT_ALARM` (API 31+, xin qua `Settings.ACTION_REQUEST_SCHEDULE_EXACT_ALARM` — không có dialog runtime), fallback về inexact khi user từ chối. Thêm preference "Exact reminders" trong Settings hiển thị trạng thái granted/denied, tap để mở màn xin quyền hệ thống. Khi user quay lại từ màn đó với quyền vừa được cấp, tự động gọi `reminderAlarmManager.updateAllAlarms()` để nâng cấp alarm đã lỡ đặt inexact trước đó, không phải chờ tới lần nổ tự nhiên tiếp theo mới được reschedule. Không xin `USE_EXACT_ALARM` (cần khai báo core-functionality alarm-clock/calendar với Play Store, app này không đủ điều kiện). `compileDevDebugKotlin` + `./gradlew test` + `compileProductionReleaseKotlin` + `assembleDevDebug` đều PASS. Chưa smoke-test device (user chọn push thẳng).
