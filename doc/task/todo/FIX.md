...
| FIX-M17 | ⚠️ **THỬ SỬA RỒI REVERT (2026-08-17, xem device smoke test log)** — mutate in-place thay vì `.copy()` khiến DiffUtil không phát hiện thay đổi gì (identity-only `===`), RecyclerView không rebind, UI không cập nhật dù data đúng — tệ hơn bug gốc. Đã revert về `.copy()`. Cần fix đúng cách bằng stable-id diffing hoặc `notifyItemChanged()` tường minh — để P2 backlog, không phải P1 nữa. | `ui/edit/EditVM.kt:616-624` (commit `e37076b`) | claude-bypass (FIX-027) | M (cần thiết kế lại, không phải 1-dòng) |
